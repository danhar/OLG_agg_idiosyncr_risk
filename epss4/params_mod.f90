...
!-------------------------------------------------------------------------------------------------
    ...
    logical ,protected :: ccv, surv_rates, def_contrib, partial_equilibrium, twosided_experiment, collateral_constraint, kappa_in_01,&
                          ...
                          save_all_iterations, detailed_euler_errs, normalize_coeffs, kappa_search_robust, opt_zbrak, tau_experiment, welfare_decomposition
...
! Some variables might be set that are not explicitly mentioned in the calibration file
    ...
    save_all_iterations=.false.; detailed_euler_errs=.false.; normalize_coeffs=.true.; kappa_search_robust=.false.; opt_zbrak=.false.; tau_experiment=.false.; welfare_decomposition = .true.
    ! Character
...
subroutine ReadCalibration(calib_name)
    ...
    openif: if (istat==0) then
        ...
        do
            ...
            case ('kappa_search_robust')
                read (parval,*) kappa_search_robust
            case ('opt_zbrak')
...
! Put checks into the modules / objects they belong to? Would probably destroy pure in some cases?
    ...
    select case(mean_return_type)
     ...
     select case(calib_targets)
        case('paper', 'presentation', 'computation', 'pc', 'std_w', 'del_mean', 'sharpe', 'nosharpe', 'no_ep')
        ! continue
        case default
            print*, 'ERROR: calib_targets must take one of the following values:'
            print*, 'paper, presentation, computation, pc, std_w, del_mean, sharpe, nosharpe, no_ep'
            call critical_stop
    end select
...
219 format(a16, es8.2)
    ...
    write(21,'(a20,l1)') ' kappa_srch_robs =  ', kappa_search_robust
