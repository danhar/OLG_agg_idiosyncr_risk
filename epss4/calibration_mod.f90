...
!-------------------------------------------------------------------------------
    ...
    pure function get_params(n, targetname)
        ...
        case('base_no_delstd')
            ...
            if (n > 2) get_params(3) = pi1_delta
        case('cal_zeta')
            get_params(1) = zeta_std
            if (n > 1) get_params(2) = del_mean
            if (n > 2) get_params(3) = beta
            if (n > 3) get_params(4) = pi1_delta
        case('no_AR')
            get_params(1) = del_mean
            if (n > 1) get_params(2) = beta
       case('base_no_beta')
            get_params(1) = del_std
            if (n > 1) get_params(2) = del_mean
            if (n > 2) get_params(3) = pi1_delta
        case default
            get_params(1) = beta
...
!-------------------------------------------------------------------------------
    ...
    subroutine set_params(param_vec, targetname)
        ...
        case('no_beta','sharpe_ratio','equity_premium')
            ...
            if (n > 4) call params_set('beta',param_vec(5))
        case('base_no_delstd')
            call params_set('beta',param_vec(1))
            if (n > 1) call params_set('del_mean',param_vec(2))
            if (n > 2) call params_set('pi1_delta',param_vec(3))
        case('cal_zeta')
            call params_set('zeta_std',param_vec(1))
            if (n > 1) call params_set('del_mean',param_vec(2))
            if (n > 2) call params_set('beta',param_vec(3))
            if (n > 3) call params_set('pi1_delta',param_vec(4))
        case('no_AR')
            call params_set('del_mean',param_vec(1))
            if (n > 1) call params_set('beta',param_vec(2))
        case('base_no_beta')
            call params_set('del_std',param_vec(1))
            if (n > 1) call params_set('del_mean',param_vec(2))
            if (n > 2) call params_set('pi1_delta',param_vec(3))
        case default
            call params_set('beta',param_vec(1))
...
!-------------------------------------------------------------------------------
    ...
    pure function model_targets(n, simvars, targetname)
        ...
        character(len=*)      ,intent(in) :: targetname
        type(tStats) :: K_Y, I_Y, ex_ret, r, rf, zeta, netwage, cons_grow
        ...
        case('nosharpe','baseline','base_K_Y_3')
            model_targets(1) = rf%avg_exerr_()
            if (n > 1) model_targets(2) = cons_grow%std_()
        ...
        case('cal_zeta')
            model_targets(1) = cons_grow%std_()
            if (n > 1) model_targets(2) = rf%avg_exerr_()
            if (n > 2) model_targets(3) = K_Y%avg_exerr_()
            if (n > 3) model_targets(4) = corr(zeta,r)
        case('I_Y')
            model_targets(1) = rf%avg_exerr_()
        ...
        case('no_beta')
            ...
            if (n > 1) model_targets(2) = cons_grow%std_()
            if (n > 2) model_targets(3) = corr(zeta,r)
            if (n > 3) model_targets(4) = ex_ret%avg_exerr_()/ex_ret%std_()
        case('sharpe_ratio')
            model_targets(1) = rf%avg_exerr_()
            if (n > 1) model_targets(2) = cons_grow%std_()
            if (n > 2) model_targets(3) = corr(zeta,r)
            if (n > 3) model_targets(4) = ex_ret%avg_exerr_()/ex_ret%std_()
            if (n > 4) model_targets(5) = K_Y%avg_exerr_()
        case('equity_premium')
            model_targets(1) = r%avg_exerr_()
            if (n > 1) model_targets(2) = r%std_()
            if (n > 2) model_targets(3) = corr(zeta,r)
            if (n > 3) model_targets(4) = ex_ret%avg_exerr_()
            if (n > 4) model_targets(5) = K_Y%avg_exerr_()
        case('no_ep')
        ...
        case('base_no_beta')
            model_targets(1) = cons_grow%std_()
            if (n > 1) model_targets(2) = rf%avg_exerr_() !K_Y%avg_exerr_()
            if (n > 2) model_targets(3) = corr(zeta,r)
        case default
            model_targets(1) = K_Y%avg_exerr_()
            if (n > 1) model_targets(2) = r%std_()
